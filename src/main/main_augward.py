...
from data import load_data_fold, load_data, save_pickle, load_pickle, batch_to_dicts, dicts_to_batch, data_file_exists
from gin import DistanceGIN
from utils import fix_seed, calculate_fgwdistances, check_path
from loss import SoftCELoss, TargetLoss
...
def parse_args():
    ...
    parser.add_argument('--weighted', type=float)
    parser.add_argument('--weighted-cr', type=float)
...
def eval_acc(model, loader, device):
    ...
    for data in loader:
        data = data.to(device)
        _, _, output = model(data.x, data.edge_index, data.x, data.edge_index, data.batch, data.batch)
...
def main():
    ...
    fgw_distances_file = f"./data/pickle/{args.data}_{args.aug}_fgw_{args.alpha}.pkl"
    check_path(fgw_distances_file)
    ...
    model = DistanceGIN(num_features, num_classes, args.dropout)
    model = model.to(device)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    ce_loss = SoftCELoss() 
    target_loss = TargetLoss()

    if data_file_exists(augmented_batches_file):
        print("Loading saved augmented batches...")
        dict_batches = load_pickle(augmented_batches_file)
        augmented_batches = [dicts_to_batch(dicts) for dicts in dict_batches]
    else:
        print("Saving augmented batches...")
        ...
        dict_batches = [batch_to_dicts(batch) for batch in augmented_batches]
        save_pickle(augmented_batches_file, dict_batches)
        print("Successfully saved augmented batches...")
    ...
    if data_file_exists(augmented_batches_file) and data_file_exists(fgw_distances_file):
        print("Loading saved FGW distances...")
        fgw_distances = load_pickle(fgw_distances_file)
    else:
        print("Calculating FGW distances...")
        fgw_distances = calculate_fgwdistances(args, trn_loader, aug_loader, trn_graphs, augmented_batches, device)
        print("Saving FGW distances...")
        save_pickle(fgw_distances_file, fgw_distances)
        print("Successfully saved FGW distances...")
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for (batch_idx, trn_batch) in enumerate(trn_loader):
            trn_batch = trn_batch.to(device)

            dist, output_trn, _ = model(trn_batch.x, trn_batch.edge_index, trn_batch.x, trn_batch.edge_index, trn_batch.batch, trn_batch.batch)
            loss = ce_loss(output_trn, trn_batch.y) 
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            loss_sum += loss.item()

        for (batch_idx, (trn_batch, aug_batch)) in enumerate(zip(trn_loader, aug_loader)):
            ...
            dist, output_trn, output_aug = model(trn_batch.x, trn_batch.edge_index, aug_batch.x, aug_batch.edge_index, trn_batch.batch, aug_batch.batch)
            fgwdistance = fgw_distances[batch_idx].to(device).float()
            loss = ce_loss(output_aug, aug_batch.y) + args.weighted * target_loss(fgwdistance, dist) + args.weighted_cr * ce_loss(output_aug, output_trn)
            if args.data == "DD" or args.data == "REDDIT-BINARY":
                loss = ce_loss(output_aug, aug_batch.y) + args.weighted * target_loss(fgwdistance, dist)
